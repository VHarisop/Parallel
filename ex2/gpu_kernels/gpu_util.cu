...
void allocate_data_on_gpu(REAL **dev_A_prev, REAL **dev_A, size_t N)
{
    // Allocate GPU buffers
    unsigned int size = N * N * sizeof(REAL);
    *dev_A_prev = (REAL *) gpu_alloc(size);
    if (!(*dev_A_prev))
        error(0, "gpu_alloc() failed: %s", gpu_get_last_errmsg());

    *dev_A = (REAL *) gpu_alloc(size);
    if (!(*dev_A)) {
        cudaFree(*dev_A_prev);
        error(0, "gpu_alloc() failed: %s", gpu_get_last_errmsg());
    }

    // cudaMemset((*dev_A_prev), 0, size);
    // cudaMemset((*dev_A), 0, size);
}

void copy_data_from_cpu(REAL *gpu, REAL **host, size_t N)
{
    if (copy_to_gpu(host[0], gpu, N*N*sizeof(REAL)) < 0)
        error(0, "copy_to_gpu() failed: %s", gpu_get_last_errmsg());
}

void copy_data_to_cpu(REAL **host, const REAL *gpu, size_t N)
{
    if (copy_from_gpu(host[0], gpu, N*N*sizeof(REAL)) < 0)
