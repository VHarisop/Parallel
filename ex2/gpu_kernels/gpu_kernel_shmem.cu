...
__global__ void GPU_KERNEL_NAME(_shmem)(REAL *in, REAL *out, int N)
{
    ...
    __shared__ float u_prev_sh[32][32];
    ...
    // TODO: WARP DIVERGENCE HERE
    if (block_check) 
    {
        out[I] = 0.25f * (u_prev_sh[i-1][j] + u_prev_sh[i+1][j] + u_prev_sh[i][j-1] + u_prev_sh[i][j+1]);
    }
    else if (bound_check)
    {
        out[I] = 0.25f * (in[I - 1] + in[I + 1] + in[I - N] + in[I + N]);
    }
