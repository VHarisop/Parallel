...
{
    // FIXME: Uncomment stuff for print-debug

    /*
    FILE *tFile;
    FILE *oFile;
    tFile = fopen("ask2/test.txt", "w");
    oFile = fopen("ask2/orig.txt", "w");
    */
    int flag = 1;
    for (int i = 0; i < orig->N; i++) {
        for (int j = 0; j < orig->N; j++) {
            /*
            fprintf(tFile, "%.3lf ", (double) test->A_prev[i][j]);
            fprintf(oFile, "%.3lf ", (double) orig->A_prev[i][j]);
            */
